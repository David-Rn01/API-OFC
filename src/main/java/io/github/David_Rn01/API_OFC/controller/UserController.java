...
import io.github.David_Rn01.API_OFC.dto.*;
...
import io.github.David_Rn01.API_OFC.services.RegisterServices;
...
public class UserController {
    ...
    @Autowired
    private StudentRepository user;
    ...
    public ResponseEntity<?> registerStudent(@Valid @RequestBody StudentRegistrationDTO registrationDTO){
        ...
        try{
            ...
            StudentResponseDTO response = register.registerStudant(registrationDTO);
    ...
    public ResponseEntity<?> loginStudent(@RequestBody LoginRequestDTO loginDTO){ //Recebe os dados de "login" do corpo da requisição e armazena em loginDTO
        Student student = user.findByEmail(loginDTO.getEmail()); //consulta o "endereço eletrônico" que foi registrado em loginDTO
    ...
    public ResponseEntity<?> registerDriver(@Valid @RequestBody DriverRegistrationDTO driverRegistrationDTO){
        try{
            //Chama o "metodo" registrar estudante da classe RegisterService que foi isntanciada como register
            DriverResponseDTO response = register.registerDriver(driverRegistrationDTO);
    ...
    public ResponseEntity<?> loginDriver(@RequestBody LoginRequestDTO loginDTO){
        Driver driverByEmail = driver.findByEmail(loginDTO.getEmail());
